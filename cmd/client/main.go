...
func echo(client pb.GameServiceClient) {
	var msg string
...
func main() {
	...
	// echo(client)
	stream, err := client.PlayGame(context.Background(), &pb.PlayGameRequest{BotName: *playerName})
	if err != nil {
	...
	for event.Type != pb.GameEvent_GAME_OVER {
		...
		case pb.GameEvent_YOUR_TURN:
			...
			moveRequest := makeMove(gameState, color, stream, event.MatchId)
			response, err := client.SubmitMove(context.Background(), moveRequest)
			if err != nil {
...
func makeMove(gameState [][]pb.GameEvent_FieldState, color pb.GameEvent_PlayerColor, stream grpc.ServerStreamingClient[pb.GameEvent], matchId string) *pb.MoveRequest {
